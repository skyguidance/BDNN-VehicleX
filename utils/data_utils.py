...
def generate_dataloader(config):
    ...
    if "stage" in config["buffer"].keys() and config["buffer"]["stage"] == "CNN_numpy_generate":
        ...
        val_dataloader = DataLoader(val_dataset, batch_size=int(config["train"]["batch_size"]/2),
                                    num_workers=config["train"]["num_workers"])
        test_dataloader = DataLoader(test_dataset, batch_size=int(config["train"]["batch_size"]/2),
                                     num_workers=config["train"]["num_workers"])
