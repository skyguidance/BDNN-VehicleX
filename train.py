...
def train_model_baseline(config, train_dataloader, val_dataloader, device, model, optimizer, criterion, scheduler):
    ...
    :param scheduler: training scheduler
    """
...
def train_model_CNN_baseline(config, train_dataloader, val_dataloader, device, model, optimizer, criterion, scheduler):
    """
    CNN Model trainer
    :param config: config file
    :param train_dataloader: training dataloader
    ...
    for epoch in range(epochs):
        ...
        if scheduler:
            scheduler.step()
        # Free GPU Mem, GPU memory is not enough!
        if str(device) != "cpu":
            del image
            del y
...
def train_model(config, train_dataloader, val_dataloader, device, model, optimizer, criterion, scheduler):
    ...
    if config["train"]["task"] == "CNN":
        train_model_CNN_baseline(config, train_dataloader, val_dataloader, device, model, optimizer, criterion,
                                 scheduler)
    else:
